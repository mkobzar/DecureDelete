...
void ThrowSocketException(DWORD dwError = 0);

class CSocketException : public CException
{
	// Construction
	DECLARE_DYNAMIC(CSocketException)
public:
	CSocketException(DWORD dwError);
...
struct CHECKNONZERO_EXCEPTION
  {
  HANDLE m_Value;
  CHECKNONZERO_EXCEPTION (HANDLE i = (HANDLE) 1) { if ((m_Value = i) == 0) ThrowSystemException (); }
  HANDLE operator = (HANDLE i)            { if ((m_Value = i) == 0) ThrowSystemException (); return i; }
  operator HANDLE() const { return m_Value; }
  };
